...
Minor modification made in `encode_core` to reduce memory footprint
of input data batches by deleting and clearing GPU memory after each forward pass.

Original: infinity/libs/infinity_emb/infinity_emb/transformer/crossencoder/torch.py
(https://github.com/michaelfeil/infinity/blob/409f3a28d981354aa77ef7912b418033deb8ed67/libs/infinity_emb/infinity_emb/transformer/crossencoder/torch.py)
"""
...
from infinity_emb.transformer.acceleration import to_bettertransformer  # ignore: type
...
class CrossEncoderPatched(CrossEncoder, BaseCrossEncoder):
    ...
    def encode_core(self, features: dict[str, Tensor]):
        ...
        ### Customization Start ###
        score = out_features.detach().cpu().float().numpy().copy()
