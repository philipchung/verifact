...
from rag.vector_stores import node_dataframe_to_propositions_dataframe, nodes_to_dataframe
...
def decompose_text(
    text: str,
    use_auxillary_llm: bool = False,
    proposition_type: Literal["sentence", "claim"] = CLAIM_NODE,
    ...
    if proposition_type == "sentence":
        ...
        loop = get_event_loop()
        # Whether to use main LLM or auxillary LLM for generating atomic claims
        if use_auxillary_llm:
            llm = get_aux_llm(temperature=llm_temperature, top_p=llm_top_p)
        else:
        ...
        raise ValueError("Invalid proposition type. Please choose either 'sentence' or 'claim'.")
...
def run_verifact(
    ...
    fact_type: Literal["sentence", "claim"] = "claim",
    ...
    # NOTE: if `is_reasoning_model` is True, then the auxillary non-reasoning LLM will
    # be used to generate atomic claims.
    text_nodes = decompose_text(
        text=text,
        use_auxillary_llm=is_reasoning_model,
    ...
    propositions_df = node_dataframe_to_propositions_dataframe(texts_df).assign(
        author_type=author_type,
        proposition_type=proposition_type,
        admitdate=hadm_start,
        dischargedate=hadm_end,
    )

    score_report = None
    # EHR Fact Type is the same as Proposition Type if not Specified
    ...
    score_report: ScoreReport = judge.evaluate(
        proposition_type=proposition_type,
        fact_type=fact_type,
        show_progress=True,
    )
...
def main(
    ...
    is_reasoning_model: Annotated[
        bool,
        typer.Option(help="Whether to run VeriFact's LLM-as-a-Judge with as a reasoning model."),
    ] = False,
    subject_id: Annotated[int, typer.Option(help="Subject ID identifying patient.")] = 1084,
    author_type: Annotated[
        str, typer.Option(help=f"Input Text Author type: {LLM_AUTHOR}, {HUMAN_AUTHOR}.")
    ] = LLM_AUTHOR,
    ...
    retrieval_method: Annotated[
        str, typer.Option(help=f"Retrieval method: {DENSE}, {SPARSE}, {HYBRID}, {RERANK}.")
    ] = RERANK,
    top_n: Annotated[
    ...
    if output_file:
        ...
        if output_file.suffix == ".pkl":
            ...
            raise ValueError(
                ...
                "Please specify either '.pkl', '.feather', or '.csv'."
            )
